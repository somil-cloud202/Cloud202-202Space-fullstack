...
export const Route = createFileRoute("/projects/$projectId/kanban/")({
  ...
  if (!project) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/projects" className="text-gray-600 hover:text-gray-700 underline">
          Back to Projects
        </Link>
      </div>
    );
...
function TaskForm({
  ...
  const aiPriorityMutation = useMutation(
    trpc.suggestTaskPriority.mutationOptions({
      onSuccess: (data) => {
  ...
  const handleAIPrioritySuggestion = () => {
    if (!formData.title.trim()) {
      toast.error("Please enter a task title first");
      return;
    }
    aiPriorityMutation.mutate({
      authToken: token!,
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              ...
              disabled={!formData.title.trim() || aiPriorityMutation.isPending}
            ...
            >
              ...
              {aiPriorityMutation.isPending ? "Analyzing..." : "AI Suggest"}
            </button>
          </div>
          <select
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="bg-gray-50 rounded-b-lg p-3 space-y-3 min-h-[400px]">
        {tasks.map((task) => (
          <TaskCard key={task.id} task={task} employees={employees} isAdminOrManager={isAdminOrManager} />
        ))}
      </div>
    </div>
  );
...
function TaskCard({ task, employees, isAdminOrManager }: { task: any; employees: any[]; isAdminOrManager: boolean }) {
  const user = useAuthStore((state) => state.user);
  const token = useAuthStore((state) => state.token);
  const trpc = useTRPC();
  const queryClient = useQueryClient();
  ...
  // Allow task updates if user is admin/manager or if task is assigned to them
  const canEdit = isAdminOrManager || task.assignedToId === user?.id;
